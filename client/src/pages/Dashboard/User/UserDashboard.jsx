    ...
    const fetchAppointments = async () => {
      try {
        ...
        if (Array.isArray(response.data)) {
          ...
          const next = response.data.find(
            (appointment) => appointment.status === "upcoming"
          );
          setNextAppointment(next);
        } else {
          console.error(
            "Fetched appointments data is not an array:",
            response.data
          );
        }
      } catch (error) {
        ...
        <div className={styles.content}>
          ...
          <div className={`${styles.card3} ${styles["user-info"]}`}>
            ...
            <div className={styles["card-content"]}>
              ...
              <p>
                <strong>Sugar Level:</strong> {user.sugarLevel}
              </p>
              <p>
                <strong>Blood Pressure:</strong> {user.bloodPressure}
              </p>
              <p>
                <strong>Booked Previous Doctors:</strong>{" "}
                {user.bookedPreviousDoctors.join(", ")}
              </p>
            </div>
          </div>
          ...
          {nextAppointment && (
            <div className={`${styles.card4} ${styles["next-appointment"]}`}>
              ...
              <p>
                <strong>Doctor:</strong> {nextAppointment.doctorName}
              </p>
              <p>
                <strong>Date:</strong>{" "}
